...
In this question, the original poster (OP) provides concise and self-contained C++ code that anyone could copy and run for themselves to see the speed difference. OP is specific with his data and provides exact timing data. OP has a clear goal in finding the reason as to why processing a sorted array is faster than processing an unsorted array. This question does everything right to make it easy for experts to answer, as Raymond would say. 

Because of how well the question was presented, it led to one of the most legendary and historically iconic responses in all of Stack Overflow: the answer given by user **Mysticial**. This user gave the infamous “railroad junction” analogy to explain branch prediction, which has garnered fame as one of the most helpful and effective answers given on the entire website.

![Railroad Junction Analogy](https://i.sstatic.net/muxnt.jpg)

## The "Not-So-Smart" Way

An example of a not-so-smart question would be something like **“HELP ME CODING ERRORS!!! URGENT!!”** This hypothetical user speaks with very general language and gives no context as to what his issue is or where his issue is coming from. Statements like, “it keeps giving me an error when i try to run it. i think its a bug in the compiler,” assume fault on other factors like the compiler and don’t specify any particular error anywhere in the question. 

This user also uploads an entire project file for others to look through instead of isolating the problem. This question violates many of the principles Raymond speaks about; as a result, such a thread would be downvoted heavily and quickly closed for lack of details, leaving the developer with no help at all.
